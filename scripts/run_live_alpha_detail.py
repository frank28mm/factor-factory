...
from connectors.worldquant_brain.live_simulation import (  # noqa: E402
    classify_alpha_detail_response,
    fetch_alpha_detail,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch one live WorldQuant alpha detail record.")
...
def main() -> int:
    ...
    response = fetch_alpha_detail(target_id, args.alpha_id)
    event = {
        "event_id": f"audit-{args.candidate_id}-alpha-detail",
        ...
        "created_at": now_iso(),
        "target_id": target_id,
        "classification": classify_alpha_detail_response(response),
        "request": {
            "endpoint": f"/alphas/{args.alpha_id}",
            "method": "GET",
        },
        "response": response,
    }
    write_json(AUDIT / f"{args.candidate_id}-alpha-detail.json", event)
